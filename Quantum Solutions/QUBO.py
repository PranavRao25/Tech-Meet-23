...
<<<<<<< HEAD
    ...
    def __init__(self, inv_id):
        ...
        self.A, self.B, self.N, self.G = np.zeros_like(self.Q), np.zeros_like(self.Q), np.zeros_like(
...
<<<<<<< HEAD
        ...
        for i in range(len(self.df)):
            data = self.df.loc[i]
            ti = self.__diff(date1=self.flight["DepartureDate"], time1=self.flight["DepartureTime"],
                             date2=data["ArrivalDate"], time2=data["ArrivalTime"])
            ti2 = self.__diff(date1=self.flight["DepartureDate"], time1=self.flight["DepartureTime"],
                              date2=data["DepartureDate"], time2=data["DepartureTime"])
...
>>>>>>> 4a039d2e56e27ee6add948282ec9354f1c3fd626
        ...
        for i in range(len(self.lst)):
            ...
            if data["InventoryId"] not in self.graph:
                ...
                self.Q[i, i] = self.__diff(data["DepartureDate"], data["ArrivalDate"], data["DepartureTime"],
                                           data["ArrivalTime"])
    ...
    def quantumSolve(self) -> list:
        ...
        for i in range(3):  # 5 alternate solutions
            print(f"Alternate Flight no {i + 1}")
            ...
            qp.binary_var_list(L.diagonal().size)
            qp.minimize(linear=L.diagonal(), quadratic=F)  # matrices fed into the quadratic program
            # Quantum processing
            """
            ...
            two = TwoLocal(2, 'rx', 'cx', 'linear', reps=2, insert_barriers=True)  # an ansatz circuit
