...
class QuantumSolver:
    ...
    def __init__(self, inv_id):
        self.flight = self.df.loc[self.df[self.df['InventoryId'] == inv_id].index]
    ...
    def __preProcess(self):
        ...
        for i in range(len(self.df)):
            data = self.df.loc[i]
            ti = self.__diff(start["DepartureDate"],data["ArrivalDate"],data["DepartureTime"],data["ArrivalTime"])
